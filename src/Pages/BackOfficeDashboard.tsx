...
// Déclaration de l'interface Dashboard
interface Dashboard {
  reservation: number;
  users: number;
  revenue: number;
  partnerReservations: number;
  dailyRate: number; // Taux journalier
  dailyGrowth: number; // Croissance journalière en pourcentage
  monthlyRate: number; // Taux mensuel
  monthlyGrowth: number; // Croissance mensuelle en pourcentage
  yearlyRate: number; // Taux annuel
  yearlyGrowth: number; // Croissance annuelle en pourcentage
}
...
const BackOfficeDashboard: React.FC = () => {
    ...
    if (user) fetchData(); // Si un utilisateur est connecté, appel de fetchData pour récupérer les données
  }, [user]); // useEffect dépend de l'état de l'utilisateur pour savoir quand s'exécuter
  ...
  // Gestionnaire du champ de recherche
  ...
  const filteredData = () => {
    ...
    let data: Partial<Dashboard> = { ...dashboardData };
  ...
  const displayedData = filteredData(); // Applique le filtre et stocke les données filtrées
